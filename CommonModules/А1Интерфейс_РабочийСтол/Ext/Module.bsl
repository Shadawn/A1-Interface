...
#Если НЕ Клиент Тогда
	...
	Функция ДобавитьОписаниеКнопкиОткрытияФормы(МассивОписаний, ИмяОткрываемойФормы, Знач Заголовок = Неопределено, РодительЭлемента = Неопределено, ПередЭлементом = Неопределено, Параметры = Неопределено, Действия = Неопределено) Экспорт 
